...
const products = {
  ...
  domain: {
    title: 'Domain Custom',
    subtitle: 'Upgrade web kamu dengan domain simpel',
    items: [
      {
        title: '.my.id',
        price: 'Rp 20.000',
        period: '1 Tahun',
        popular: true,
        features: ['Bebas request nama domain', 'Contoh: azbry.my.id', 'Garansi selama masa aktif', 'Proses cepat'],
      ...
      },
      {
        ...
        period: '1 Tahun',
        popular: true,
        features: ['Bebas request nama domain', 'Contoh: febrydev.web.id', 'Garansi selama masa aktif', 'Proses cepat'],
        note: 'Untuk lebih lanjut tanyakan admin',
      },
      {
        title: '.biz.id',
        ...
        period: '1 Tahun',
        popular: false,
        features: ['Bebas request nama domain', 'Contoh: azbry.biz.id', 'Garansi selama masa aktif', 'Proses cepat'],
        note: '⚠️ Sedang OUT OF STOCK',
      },
    ],
    extra: {
      type: 'benefits',
      title: 'Keuntungan Domain',
      icon: 'fa-globe',
      items: [
        ...
        '✅ Bisa diperpanjang',
        '✅ Link jadi lebih simple & profesional',
      ],
    },
  },
};
...
function renderProducts(cat) {
  ...
  let html = `
    <div class="s-product-header-info" style="margin-bottom:16px;">
      <div style="font-family:var(--font-d);font-size:18px;font-weight:800;color:var(--text);margin-bottom:4px;">${data.title}</div>
      ${data.subtitle ? `<div style="font-size:12px;color:var(--text-3);font-family:var(--font-m);">${data.subtitle}</div>` : ''}
    </div>
  ...
  data.items.forEach((item, i) => {
    html += `
      <div class="s-product-card ${item.popular ? 'popular' : ''}" style="animation-delay:${i * 0.06}s;">
...
function logConsole() {
  ...
  console.log('%cGlassmorphism Edition · by FebryWesker', 'color:#93c5fd;font-size:13px;');
